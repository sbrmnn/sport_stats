...
module SportStats
  ...
  class HitterStats < BaseballStats
        ...
        def batting_avg_imp_calc(player_array, min_at_bats, header_index={})
            ...
            keys = batting_h_ab_hash.keys.sort # Returns array of keys which containg the start year and end year. Ie. [2009, 2010]
            start_year = keys.first
            end_year = keys.last
            return nil if (batting_h_ab_hash[start_year][:ab] < min_at_bats || batting_h_ab_hash[end_year][:ab] < min_at_bats) # (7)
